...
std::string wstr_to_str(const std::wstring& arg)
{
    ...
    size_t converted = 0;
    wcstombs_s(&converted, const_cast<char*>(res.data()), arg.length(), arg.c_str(), _TRUNCATE);
...
std::wstring str_to_wstr(const std::string& arg)
{
    ...
    size_t converted = 0;
    mbstowcs_s(&converted, const_cast<wchar_t*>(res.data()), arg.length(), arg.c_str(), _TRUNCATE);
...
#ifdef STORE_IN_LIST
        m_sections.insert(itr, pSection);
#else
		m_sections.insert(pSection);
#endif
        return pSection;
...
{
#ifdef STORE_IN_LIST
	SecIndexW::const_iterator itr;
    for(itr = m_sections.begin() ; itr != m_sections.end() ; ++itr )
    {
		if(!sSection.compare((*itr)->GetSectionName()))
			break;
    }
    return itr;
#else
...
bool CIniSectionW::SetSectionName( std::wstring sSectionName )
{
    ...
    if( m_pIniFile->_find_sec( sSectionName ) == m_pIniFile->m_sections.end() )
    {
        ...
        // Set the new map entry we know should not exist
#ifdef STORE_IN_LIST
        m_pIniFile->m_sections.insert(m_pIniFile->m_sections.end(), this);
#else
		m_pIniFile->m_sections.insert(this);
#endif
...
#endif
#ifdef STORE_IN_LIST
	KeyIndexW::const_iterator itr;
    for(itr = m_keys.begin() ; itr != m_keys.end() ; ++itr )
    {
		if(!sKey.compare((*itr)->GetKeyName()))
			break;
    }
    return itr;
#else
...
bool CIniKeyW::SetKeyName( std::wstring sKeyName )
{
    ...
    if( m_pSection->_find_key( sKeyName ) == m_pSection->m_keys.end() )
    {
        ...
        // Make the new map entry
#ifdef STORE_IN_LIST
        m_pSection->m_keys.insert(m_pSection->m_keys.end(), this);
#else
		m_pSection->m_keys.insert(this);
#endif
        return true;
